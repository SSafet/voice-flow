...
final class SystemAgentStore {
    ...
    static let specs: [SystemAgentSpec] = [
        SystemAgentSpec(
            ...
            runsOn: "Codex · 15 s, then Assistant API · 10 s",
            defaultModel: "gpt-5.6-luna",
            defaultEffort: "low",
            defaultInstructions: SystemAgentDefaults.continuityInstructions,
            supportsEffort: true,
            instructionsSource: .store,
            instructionsContract: "The model receives the current conversation, new message, and required reuse/new JSON schema separately. If Codex fails, the router uses the same instructions with the API model and key from Settings → Assistant. If neither can decide, it visibly continues the conversation."),
        SystemAgentSpec(
            kind: .speechCleanup,
            name: "Speech cleanup",
